...
export default class Dapp extends React.Component<Props, State> {
  ...
  componentDidMount = async () => {
    ...
    if (browserProvider?.isMetaMask !== true) {
      this.setError( 
        <>
          We were not able to detect <strong>MetaMask</strong>. Please install a MetaMask wallet extension on your browser. 
          {/* <strong>privacy and security</strong> a lot so we limit the wallet options on the DAPP.<br />
          <br />
  ...
  private copyMerkleProofToClipboard(): void
  {
    ...
    if (merkleProof.length < 1) {
      this.setState({
        merkleProofManualAddressFeedbackMessage: 'The given address is not whitelisted please double-check.',
      });
        ...
        {!this.isWalletConnected() || !this.isSoldOut() ?
          <div className="no-wallet">
            ...
            <div className="use-block-explorer">
              Hey, we're delighted to have <strong>you</strong> here! <span className="emoji">😃</span>
              <br></br>
              <br></br>
              Connect your wallet and get ready to receive your <strong>Scotch Noblemen!</strong> 
              <br></br>
              <br></br>

              Check out our smart contract on <a href={this.generateContractUrl()} target="_blank">{this.state.networkConfig.blockExplorer.name}</a>
              <br></br>
              Thanks for contributing to the project <span className="emoji">❤️</span>
              <br/>
            </div>
          </div>
          : null}
